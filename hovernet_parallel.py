...
def run_infer_load_parallel(dl):
    start_load = time.perf_counter()

    mdl = getmodel().half()
    hovernet = torch.nn.parallel.DataParallel(mdl,device_ids = [0,1,2,3,4,5,6,7])
    ...
    for idx,batch in enumerate(dl):
        ...
        patch_imgs_gpu = batch.permute((0,3,1,2)).half()
                
        out.append(hovernet(patch_imgs_gpu))
        if idx%10==0:
            print(idx,time.perf_counter()-start_i)
...
def run_infer_load(dl,device):
    ...
    for idx,batch in enumerate(dl):
        ...
        out.append(mdl_gpu(patch_imgs_gpu))
        if idx%10==0:
...
def main():
    ...
    if num_loaders > 1:
        ...
        run_infer_load_parallel(loaders[0])
